...
public:
	...
	public:
		...
		void							addToBoundingBox(glm::vec3 point, ci::Rectf& boxToEdit);
	...
	void							setTriggeredCallback( const std::function<void (const glm::vec2 cent)> &func );
	...
	void							setTriggerableCallback( const std::function<void (const bool triggerable, const glm::vec2 cent)> &func );
...
private:
	...
	int								closeToCluster(glm::vec3 pos);

	int								findCluster(int fingerId, glm::vec3 curPos);

	std::function<void (const glm::vec2)>				mTriggeredFunction;
	std::function<void (const bool, const glm::vec2)>	mTriggerableFunction;
