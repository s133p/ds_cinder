...
void FiveFingerCluster::setTriggeredCallback(  const std::function<void (const glm::vec2)> &func )
{
	mTriggeredFunction = func;
}

void FiveFingerCluster::setTriggerableCallback(  const std::function<void (const bool, const glm::vec2)> &func )
{
...
int FiveFingerCluster::closeToCluster(glm::vec3 pos){
...
int FiveFingerCluster::findCluster(int fingerId, glm::vec3 curPos){
	for (int i = 0; i < mClusters.size(); i++){
		Cluster& c = mClusters[i];
		for (auto it = c.mTouches.begin(); it < c.mTouches.end(); ++it){
			if((*it).mFingerId == fingerId){
				(*it).mCurrentGlobalPoint = glm::vec3(curPos.x, curPos.y, 0.0f);
				return i;
			}
		}
	}
	...
	if(ti.mPhase == ds::ui::TouchInfo::Added){
		...
		if(clustyMcClustClust.mTriggerable){
			if(clustyMcClustClust.mBoundingBox.getWidth() > mBoundingBoxSize || clustyMcClustClust.mBoundingBox.getHeight() > mBoundingBoxSize){
				...
				if(mTriggerableFunction) mTriggerableFunction(false, glm::vec2(clustyMcClustClust.mBoundingBox.getCenter().x, clustyMcClustClust.mBoundingBox.getCenter().y));
			}
		} else {
			if(clustyMcClustClust.mBoundingBox.getWidth() < mBoundingBoxSize 
				...
				if(mTriggerableFunction) mTriggerableFunction(true, glm::vec2(clustyMcClustClust.mBoundingBox.getCenter().x, clustyMcClustClust.mBoundingBox.getCenter().y));
			}
		}
		...
		if (clustyMcClustClust.mTouches.size() == 0){
			...
			if (clustyMcClustClust.mTriggerable){
				if (mTriggeredFunction) mTriggeredFunction(glm::vec2(clustyMcClustClust.mBoundingBox.getCenter().x, clustyMcClustClust.mBoundingBox.getCenter().y));
				if (mTriggerableFunction) mTriggerableFunction(false, glm::vec2(clustyMcClustClust.mBoundingBox.getCenter().x, clustyMcClustClust.mBoundingBox.getCenter().y));
				clustyMcClustClust.mTriggerable = false;
			}
...
void FiveFingerCluster::Cluster::addToBoundingBox(glm::vec3 p, ci::Rectf& boxToEdit){
	if(!boxToEdit.contains(glm::vec2(p.x, p.y))){
		boxToEdit.include(glm::vec2(p.x, p.y));
	}
}
