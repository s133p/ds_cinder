...
void Layer::renderCircle(const Input& ip, RenderCircleParams& p) const
{
	...
	glm::dvec2		offset = mOffset.getValue(ip);
...
void Layer::setScale(const ScaleMode& mode, const double amount)
{
	...
	if (mode == SCALE_FIT) {
		...
		mScaleFn = [this](const RenderCircleParams& p, const glm::dvec2& offset)->double{
		...
		mScaleMode = SCALE_MULTIPLY;
		mScaleFn = [this](const RenderCircleParams& p, const glm::dvec2&)->double{return p.mMaxDist * this->mScale;};
	}
}
