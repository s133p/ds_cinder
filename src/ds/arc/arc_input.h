...
public:
	...
	glm::dvec2				getVec2(const size_t index, const glm::dvec2& defaultValue) const;

	void					addColor(const ci::ColorA&);
	void					addFloat(const double);
	void					addVec2(const glm::dvec2&);
...
class Vec2Param {
public:
	Vec2Param(const glm::dvec2& v = glm::dvec2(0.0f, 0.0f));

	glm::dvec2		getValue(const Input&) const;
