...
glm::dvec2 Input::getVec2(const size_t index, const glm::dvec2& defaultValue) const {
...
void Input::addVec2(const glm::dvec2& v) {
	mVec2.push_back(v);
}
	...
	try {
		while (buf.canRead<char>()) {
			...
			if (att == COLOR_ATT) {
				...
				mVec2.push_back(buf.read<glm::dvec2>());
			} else if (att == END_ATT) {
				return true;
			} else {
				return false;
 ...
 * ds::arc::Vec2Param
 */
Vec2Param::Vec2Param(const glm::dvec2& value)
		: mValue(value)
		, mInputIndex(-1) {
}

glm::dvec2 Vec2Param::getValue(const Input& input) const {
	if (mInputIndex < 0) return mValue;
