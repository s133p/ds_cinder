...
bool MeshFileLoader::Load( const std::string &filename, unsigned chunk /*= 256*/ )
{
    ...
    if ( filestr.is_open() )
    {
        ...
        if ( mNumVert > 0 )
            mVert = new glm::vec3[mNumVert];
        if ( mNumNorm > 0 )
            mNorm = new glm::vec3[mNumNorm];
        if ( mNumIndices > 0 )
            mIndices = new unsigned[mNumIndices];
        if ( mNumTex > 0 )
            mTex = new glm::vec2[mNumTex];
        if ( mNumVert > 0 )
            ChunkRead( filestr, (char*)mVert, mNumVert*sizeof(glm::vec3), chunk );
        if ( mNumNorm > 0 )
            ChunkRead( filestr, (char*)mNorm, mNumNorm*sizeof(glm::vec3), chunk );
        if ( mNumIndices > 0 )
            ChunkRead( filestr, (char*)mIndices, mNumIndices*sizeof(unsigned), chunk );
        if ( mNumTex > 0 )
            ChunkRead( filestr, (char*)mTex, mNumTex*sizeof(glm::vec2), chunk );
        filestr.close();
...
void MeshFileLoader::Write( const std::string &filename, unsigned chunk /*= 256*/ )
{
    ...
    if ( filestr.is_open() )
    {
        int count = 3*sizeof(int)+mNumVert*sizeof(glm::vec3)+mNumNorm*sizeof(glm::vec3)+mNumIndices*sizeof(int)+mNumTex*sizeof(glm::vec2);
        //Header
        int header = 0x1ee7ed;
        ...
        if ( mNumVert > 0 )
            ChunkWrite( filestr, (char*)mVert, mNumVert*sizeof(glm::vec3), chunk );
        if ( mNumNorm > 0 )
            ChunkWrite( filestr, (char*)mNorm, mNumNorm*sizeof(glm::vec3), chunk );
        if ( mNumIndices > 0 )
            ChunkWrite( filestr, (char*)mIndices, mNumIndices*sizeof(unsigned), chunk );
        if ( mNumTex > 0 )
            ChunkWrite( filestr, (char*)mTex, mNumTex*sizeof(glm::vec2), chunk );
        filestr.close();
    }
}

void MeshFileLoader::setVerts( const std::vector<glm::vec3> &verts )
{
    if ( mVert )
    {
        delete [] mVert;
...
void MeshFileLoader::setTexs( const std::vector<glm::vec2> &texs )
{
    if ( mTex )
    {
        delete [] mTex;
...
void MeshFileLoader::setNorms( const std::vector<glm::vec3> &norms )
{
    if ( mNorm )
    {
        delete [] mNorm;
