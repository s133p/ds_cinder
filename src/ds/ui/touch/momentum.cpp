...
void Momentum::update(const ds::UpdateParams&) {
	...
	if (mActive) {
		...
		glm::vec3		curr_pos(mSprite->getPosition());
	...
	glm::vec2		curPos = glm::vec2(mSprite->getPosition());
	curPos.x += absWidth/2.0f - mSprite->getCenter().x * absWidth;
	curPos.y += absHeigh/2.0f - mSprite->getCenter().y * absHeigh;

	glm::vec2		delta = glm::vec2(curPos.x - mLastPosition.x, curPos.y - mLastPosition.y);
...
const glm::vec2& Momentum::getVelocity() {
	return mVelocity;
}
