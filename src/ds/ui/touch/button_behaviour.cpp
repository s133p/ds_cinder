...
void			anim_scale_to(ds::ui::Sprite& s, const float scale, const float duration) {
	s.mAnimScale.stop();
	s.getEngine().getTweenline().apply(s, s.ANIM_SCALE(), glm::vec3(scale, scale, 1.0f), duration, ci::easeInOutQuad);
}
...
bool ButtonBehaviour::ownerContains(const glm::vec3& point) const {
	...
	if (mIsSetToScale) {
		ds::ui::Sprite::LockScale	lock(mOwner, glm::vec3(1.0f, 1.0f, 1.0f));
