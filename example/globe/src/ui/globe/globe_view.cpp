	...
	, mYMomentum(g)
{
	...
	mGlow->getUniform().setVec4f("size", glm::vec4(mGlow->getWidth(), mGlow->getHeight(), 0, 0));
	addChild(*mGlow);
	mGlow->sendToBack();
...
void GlobeView::drawClient(const glm::mat4 &trans, const ds::DrawParams &drawParams){
	if(visible()) {
		mGlobeMesh.setRotation(glm::vec3(mGlobeMesh.getRotation().x + mXMomentum.getDelta(), mGlobeMesh.getRotation().y + 0.1f + mYMomentum.getDelta(), 0.0f));
		ds::PerspCameraParams p = mEngine.getPerspectiveCamera(0);
		mGlobeMesh.getUniform().setVec4f("camPos", glm::vec4(p.mPosition, 1.0f));
