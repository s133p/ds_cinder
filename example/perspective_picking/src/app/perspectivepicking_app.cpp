...
PerspectivePicking::PerspectivePicking()
	: inherited(ds::RootList()
								...
								.perspFov(60.0f)
								.perspPosition(glm::vec3(0.0, 0.0f, 960.0f))
...
void PerspectivePicking::keyDown(ci::app::KeyEvent event){
	...
	if(event.getChar() == KeyEvent::KEY_r){
		...
		moveCamera(glm::vec3(moveAmount, 0.0f, 0.0f), moveTarget);
	} else if(event.getCode() == KeyEvent::KEY_LEFT){
		moveCamera(glm::vec3(-moveAmount, 0.0f, 0.0f), moveTarget);
	} else if(event.getCode() == KeyEvent::KEY_UP){
		moveCamera(glm::vec3(0.0f, -moveAmount, 0.0f), moveTarget);
	} else if(event.getCode() == KeyEvent::KEY_DOWN){
		moveCamera(glm::vec3(0.0f, moveAmount, 0.0f), moveTarget);
	} else if(event.getCode() == KeyEvent::KEY_RIGHTBRACKET){
		moveCamera(glm::vec3(0.0f, 0.0f, moveAmount), moveTarget);
		...
		moveRoot(glm::vec3(0.0f, moveAmount, 0.0f));
	} else if(event.getCode() == KeyEvent::KEY_COMMA){
		moveRoot(glm::vec3(-moveAmount, 0.0f, 0.0f));
		...
		rooty.setPosition(glm::vec3(0.0f, 0.0f, 0.0f));
		ds::PerspCameraParams p = mEngine.getPerspectiveCamera(mDebugCamera);
		p.mTarget = glm::vec3(0.0f, 0.0f, p.mTarget.z);
		p.mPosition = glm::vec3(0.0f, 0.0f, p.mPosition.z);
		mEngine.setPerspectiveCamera(mDebugCamera, p);
...
void PerspectivePicking::moveRoot(const glm::vec3& deltaMove){
	ds::ui::Sprite& rooty = mEngine.getRootSprite(mDebugCamera);
	rooty.move(deltaMove);
	std::cout << "Moving root " << mDebugCamera << " pos: " << rooty.getPosition() << std::endl;
}

void PerspectivePicking::moveCamera(const glm::vec3& deltaMove, const bool moveTarget){
	ds::PerspCameraParams p = mEngine.getPerspectiveCamera(mDebugCamera);
	if(moveTarget){
		p.mTarget += deltaMove;
